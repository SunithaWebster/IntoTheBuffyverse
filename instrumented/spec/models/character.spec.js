...
describe("Character model", () => {
  ...
  it("has a character description", () => {
    const character = new Character({
      character_description:
        "Buffy Anne Summers was a Slayer activated in the late 20th century. She moved to Sunnydale with her mother, where she acted as guardian of the Hellmouth for seven years before the town's destruction in mid-2003.",
    });
  ...
  it("can store relationship details of a character", () => {
    const character = new Character({
      character_relationships_with: {
        Angel: "Love Interest",
        "Joyce Summers": "Mother",
        "Rupert 'Ripper' Giles": "Watcher",
      },
    });
    expect(character.character_relationships_with["Joyce Summers"]).toContain(
      "Mother"
    );
  });
  ...
  it("has an image url for the character", () => {
    const character = new Character({
      images_url:
        "https://static.wikia.nocookie.net/buffy/images/9/92/C1.jpg/revision/latest?cb=20130212141636",
    });
    expect(character.images_url).toEqual(
      "https://static.wikia.nocookie.net/buffy/images/9/92/C1.jpg/revision/latest?cb=20130212141636"
    );
  });
  ...
  it("has trivia information about the character", () => {
    ...
    expect(character.trivia).toEqual(
      "The scene with Buffy and Giles at the Bronze was Anthony Head's audition scene."
    );
  });
