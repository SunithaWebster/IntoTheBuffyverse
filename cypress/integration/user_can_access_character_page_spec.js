describe("Character page", () => {
  ...
  it("checks that it has a title"),
    () => {
      cy.get("h3").and("include", "Characters: Buffy Anne Summers");
    };
  ...
  it("checks that the menu link for wikipedia works"),
    () => {
      cy.get('div[class="dropdown"]');
      cy.url().should(
        "include",
        'a[href="https://en.wikipedia.org/wiki/Buffy_the_Vampire_Slayer"]'
      );
    };

  it("checks that the character information is present"),
    () => {
      cy.get('div[class="character"]')
        .find("p")
        .should(
          "include",
          "in Los Angeles, but, due to an incident involving a gang of vampires at Hemery High School"
        );
    };
});
